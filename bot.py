...
async def reset(update,context):
    ...
    cursor.execute("DELETE FROM stats WHERE user_id=?", (user,))
    cursor.execute("DELETE FROM my_words WHERE user_id=?", (user,))
    conn.commit()

    await update.message.reply_text("Reset tehtud")
